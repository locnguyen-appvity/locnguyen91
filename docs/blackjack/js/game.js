function init() {
	...
	var game = {
		...
		buttons: [
			new Button("Kéo", "#fff", 100, 100, () => player.hit()),
			new Button("Dằn", "#fff", 200, 100, () => player.stand()),
			new Button("Chơi", "#fff", 935, -430, () => game.go()),
			new Button("", "#fff", 100, -80, () => player.insure()),
			new Button("", "#fff", 100, -40, () => player.double()),
			new Button("", "#fff", 100, 0, () => player.giveUp()),
			new Button("Game", "#fff", 100, -490, () => game.reset()),
		],
		...
		startScreen: function () {
			...
			createjs.Sound.registerSound(
				"assets/Bonus/chipsCollide1.ogg",
				"chip"
			);
			if (localStorage.getItem("BlackJackJs-userName")) {
				player.name.value = localStorage.getItem(
					"BlackJackJs-userName"
				);
				player.funds = localStorage.getItem("BlackJackJs-funds");
				player.chips = JSON.parse(
					localStorage.getItem("BlackJackJs-chips")
				);
				this.start();
			} else {
				...
				nameInput._hiddenInput.style.left =
					nameInput.x +
					stage.canvas.offsetLeft +
					nameInput._padding +
					"px";
				nameInput._hiddenInput.style.top =
					nameInput.y +
					stage.canvas.offsetTop +
					nameInput._padding +
					"px";
		...
		start: function () {
			...
			if (this.startContainer) this.startContainer.removeAllChildren();
			this.message.init();
		...
		go: function () {
			if (player.dealt && !this.inProgress) {
				...
				this.new();
			} else if (!player.dealt) game._alert(messages.warning.bet);
		},
		...
		deckValue: function (deck) {
			...
			deck.forEach(function (card) {
				if (card.value >= 2 && card.value < 11) total += card.value;
		...
		distributeCard: function (to, hidden = false) {
			...
			if (to === "bank") bank.deck.push(card);
			else if (to === "player") player.deck.push(card);
		...
		displayCard: function (card, owner) {
			...
			if (owner === "bank") {
				...
				if (this.deckValue(player.deck) > 21) player.lose();
			}
		},
		...
		addChips: function () {
			...
			} else player.chipsContainer.removeAllChildren();

			var base = {x: 100, y: 45};
			for (var chip in player.chips) {
				for (let i = 0; i < player.chips[chip]; i++) {
					var chipImg = new createjs.Bitmap(
		...
		check: function () {
			...
			if (bank.blackjack && player.blackjack) return player.draw();
			else if (bank.blackjack) return player.lose();
			else if (player.blackjack) return player.win();

			if (bankScore > 21) player.win();
			else if (bankScore >= 17 && bankScore <= 21) {
				if (playerScore > bankScore) player.win();
				else player.lose();
			}
		},
	};
	...
	var player = {
		...
		hit: function () {
			if (this.betted) {
				...
				game.distributeCard("player");
			} else game._alert(messages.warning.bet);
		},

		stand: function () {
			if (!this.betted) return game._alert(messages.warning.bet);
			game.inProgress = true;
			bank.play();
		},
		...
		insure: function () {
			...
			} else game._alert(messages.warning.insurance);
		},
		...
		double: function () {
			if (game.inProgress && this.deck.length === 2 && !this.doubled) {
				if (this.funds >= this.dealt) {
					...
					for (var chip in game.dealt)
						if (game.dealt[chip]) game.dealt[chip] *= 2;
					player.fundsText.update();
				} else game._alert(messages.warning.funds);
			} else game._alert(messages.warning.double);
		},
		...
		giveUp: function () {
			if (
				...
				game.addChips();
				game.end();
			} else game._alert(messages.warning.giveUp);
		},
		...
		lose: function () {
			...
			setTimeout(function () {
				...
				if (player.funds <= 0) return game.over();
				game.end();
			}, 2000);
		},
